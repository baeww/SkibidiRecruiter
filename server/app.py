...
from agent.interface import add_knowledge, interview, scoreApplicant
...
def get_score():
    ...
    # score, reason = scoreApplicant(job_id, applicant_id) 
    ...
    try:
        ...
        result = []
        for entry in entries:
            score, reason = scoreApplicant(job_id, entry['agentID'])
            result.append({
                'score': score,
                'reason': reason
            })
